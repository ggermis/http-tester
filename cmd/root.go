...
func init() {
	...
	rootCmd.PersistentFlags().StringVarP(&cli.Option.OutputFormat, "output", "o", cli.OUTPUT_DOT, "Output format to use. (dot, csv, detail, null)")
	rootCmd.PersistentFlags().BoolVarP(&cli.Option.ForceTLSHandshake, "tls-handshake", "t", false, "Do TLS handshake on every request")
	...
	rootCmd.PersistentFlags().IntVar(&cli.Option.SlowRequests, "slow-requests", 15000, "Indicate slow requests in the dot outputter with an 'S'. Specified in ms")
}
